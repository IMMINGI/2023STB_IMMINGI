...
table(is.na(foodshop$open_date))#결측치 64개 있음
#na값 제외
foodshop <- foodshop %>%
  filter(open_date != '') %>%
  select(name,type,status,open_date,close_date,address)
...
foodshop$district <- ifelse(foodshop$district%in%c(" 밀양"," 영암","106","번지","시 강","시 계","시 관","시 금","시 남","시 노","시 마","시 미","시 서","시 용","시 은","회","여주군"),NA,foodshop$district)#이상치제거 (밀양,영암 각 1개는 경기도 아닌데 잘못 포함되서 지움,여주군 1개 여주군은 없어서 지움)
table(foodshop$district)#이상치 확인
...
foodshop %>%
  filter(!is.na(open_date)&status=="영업") %>% #결측치제거, 영업데이터 추출
  filter(type%in%c("기타","경양식","분식","일식","중국식","호프/통닭","한식"))%>%
